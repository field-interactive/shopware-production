...
if (isHotMode) {
     ...
     *
    */
    const scssEntryFilePath = path.resolve(projectRootPath, 'var/theme-entry.scss').replace(/\\/g, '/');
    const scssDumpedVariables = path.resolve(projectRootPath, 'var/theme-variables.scss').replace(/\\/g, '/');
    const scssEntryFileContent = (() => {
        ...
        const collectedImports = [dumpedVariablesImport, assetOverrides, ...themeFiles.style.map((value) => {
            const importPath = value.filepath.replace(/\\/g, '/');
