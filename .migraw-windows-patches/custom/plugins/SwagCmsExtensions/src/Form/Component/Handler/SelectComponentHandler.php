...
use Swag\CmsExtensions\Form\Route\Validation\Constraint\AtLeastOneOf;
...
class SelectComponentHandler extends AbstractComponentHandler
{
    ...
    public function getValidationDefinition(FormGroupFieldEntity $field, SalesChannelContext $context): array
    {
        ...
        if (isset($config['options'])) {
            $choices = \array_map('strval', \array_keys($config['options']));
            $field->isRequired() ?: ($choices[] = "");
            $validations[] = new Choice($choices);
        }
